...
jade::Mesh::Mesh(GLfloat* vertices, GLsizei vertices_size, MeshFormat fmt) : ibo(0), indexed(false) {
    ...
    switch (fmt) {
        case MeshFormat::P:
            index_ct = vertices_size / sizeof(float) / 3;
            glVertexAttribPointer(0, 2, GL_FLOAT, GL_FALSE, 2 * sizeof(float), (void*)0);
            glEnableVertexAttribArray(0);
            break;
        case MeshFormat::PT:
            index_ct = vertices_size / sizeof(float) / 5;
...
jade::Mesh jade::Mesh::triangle(float b, float h, bool t) {
    if (!t) {
        ...
        return Mesh(vertices, 9 * sizeof(float), MeshFormat::P);
    } else {
        ...
        add_malloc(vertices, true);
        return Mesh(vertices, 15 * sizeof(float), MeshFormat::PT);
    }
}
...
jade::Mesh jade::Mesh::circle(float r, int n, bool t) {
    if (!t) {
        ...
        for (int i = 2, a = 0; i < n + 2; i++, a += 2 * M_PI / n) {
        ...
        for (int i = 4, a = 0; i < n + 4; i++, a += 2 * M_PI / n) {
            vertices[i + 0] = r * cosf(a);
            vertices[i + 1] = r * sinf(a);
